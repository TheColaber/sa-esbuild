        ...
        callback: () => {
          ...
          for (const topBlock of topBlocks) {
            ...
            const blocksInColumn =
              (selectedCoords.x <= blockCoords.x &&
                blockCoords.x < selectedRight) ||
              (blockCoords.x <= selectedCoords.x &&
                selectedCoords.x < blockRight);

            const shouldShiftY =
              blocksInColumn && blockCoords.y > selectedBottom;
            const shouldShiftX =
              !shouldShiftY &&
              blockCoords.x >= selectedCoords.x &&
              blockBottom >= selectedCoords.y;
            ...
            const diffX =
              ADJUSTED_MOVE_X -
              blockCoords.x +
              Math.round(selectedRight / spacing) * spacing;
            const diffY =
              ADJUSTED_MOVE_Y -
              blockCoords.y +
              Math.round(selectedBottom / spacing) * spacing;
