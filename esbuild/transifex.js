...
const txPull = async function (
  ...
  if (localeCode === SOURCE_LOCALE) {
    url = await transifexApi.ResourceStringsAsyncDownload.download({
      resource,
    });
  } else {
    const language = {
  ...
  for (let i = 0; i < 5; i++) {
    try {
      ...
      process.stdout.write(
        `got ${e.message}, retrying after ${i + 1} failed attempt(s)\n`,
      );
    }
  }
 ...
 */
const txCreateResource = async function (
  project,
  { slug, name, i18nType, sourceStrings },
) {
  const i18nFormat = {
    data: {
